...
float time_a_search(kdtree2::KDTree *tree, int nn, int nsearch)
{
  ...
  {
    for (int j = 0; j < dim; j++)
      query[j] = random_variate();
    tree->n_nearest(query, nn, result);
  }

  t1 = clock();

  return (static_cast<float>(static_cast<double>(t1 - t0) / static_cast<double>(CLOCKS_PER_SEC)));
}
  ...
  std::cout << "Tree created, now testing against brute force...";
  {
    ...
    for (int i = 0; i < 50; i++)
    {
      ...
      for (int k = 0; k < nn; k++)
      {
        ...
        {
          std::cout << "Mismatch! nn=" << k << " brute=[" << resultbrute[k].dis << "," << resultbrute[k].idx << "] tree=[" << result[k].dis << "," << result[k].idx << "]\n";
        }
      }
    }
  }
